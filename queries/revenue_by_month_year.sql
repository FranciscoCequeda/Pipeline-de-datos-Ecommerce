...
SELECT strftime('%m', s.order_delivered_customer_date) AS month_no,
    case
        strftime("%m", s.order_delivered_customer_date)
        WHEN '01' THEN 'Jan'
        WHEN '02' THEN 'Feb'
        WHEN '03' THEN 'Mar'
        WHEN '04' THEN 'Apr'
    ...
    SUM(
        CASE
            WHEN strftime('%Y', s.order_delivered_customer_date) = '2016' THEN s.payment_value
            ELSE 0.00
        END
    ) AS Year2016,
    SUM(
        CASE
            WHEN strftime('%Y', s.order_delivered_customer_date) = '2017' THEN s.payment_value
            ELSE 0.00
        END
    ) AS Year2017,
    SUM(
        CASE
            WHEN strftime('%Y', s.order_delivered_customer_date) = '2018' THEN s.payment_value
            ELSE 0.00
        END
    ) AS Year2018
FROM stage s
