...
with delivery_times as (
	select julianday(oo.order_delivered_customer_date) - julianday(oo.order_purchase_timestamp) as real_time,
		...
		case
			strftime("%m", oo.order_purchase_timestamp)
			WHEN '01' THEN 'Ene'
			WHEN '02' THEN 'Feb'
			WHEN '03' THEN 'Mar'
			WHEN '04' THEN 'Abr'
...
select d.month_no,
	...
	avg(
		case
			when d.year_date = "2016" then d.real_time
		end
	) as Year2016_real_time,
	avg(
		case
			when d.year_date = "2017" then d.real_time
		end
	) as Year2017_real_time,
	avg(
		case
			when d.year_date = "2018" then d.real_time
		end
	) as Year2018_real_time,
	avg(
		case
			when d.year_date = "2016" then d.estimated_time
		end
	) as Year2016_estimated_time,
	avg(
		case
			when d.year_date = "2017" then d.estimated_time
		end
	) as Year2017_estimated_time,
	avg(
		case
			when d.year_date = "2018" then d.estimated_time
		end
